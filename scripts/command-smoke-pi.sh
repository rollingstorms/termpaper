...
usage() {
  cat <<'USAGE'
Usage: scripts/command-smoke-pi.sh [--display mock|debug|waveshare]
...
run_smoke() {
  local command="$1"
  ssh -tt "${PI_HOST}" "cd '${PI_DIR}' && cargo run -- --display '${DISPLAY}' --panel waveshare3-in-g --command '${command}'"
}
